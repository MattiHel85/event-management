...
import { submitFeatureRequest } from "../lib/api/featureRequests";

const CATEGORIES = ["Event Planning", "Budget", "Guest Management", "Integrations", "Reports", "Other"];
    ...
    <main className="max-w-3xl mx-auto pt-4 pb-24">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm text-slate-600 mb-1.5">Contact (optional)</label>
            <input
              type="text"
              value={form.contact}
          ...
          {status === "success" && <p className="text-emerald-600 text-sm">Thanks! Your feature request was submitted.</p>}
          {status === "error" && <p className="text-red-500 text-sm">Could not submit right now. Please try again.</p>}
