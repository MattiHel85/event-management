...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 flex flex-col">
      <header className="min-h-16 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2 px-4 py-2 sm:flex-nowrap sm:px-6 md:px-8">
        <span className="font-semibold text-slate-900 tracking-tight text-sm sm:text-base">Event Management</span>
        <div className="flex items-center justify-end gap-2 sm:gap-3">
          ...
          {user ? (
            <Link
              to="/dashboard"
              className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-2 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 max-w-[12rem] sm:max-w-none sm:px-3"
            >
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 text-center py-14 sm:py-20 md:py-24">
        <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-slate-900 mb-4 sm:mb-6 leading-tight max-w-3xl">
          Everything you need to <span className="text-blue-600">run great events</span>
        </h1>
        <p className="text-slate-500 text-base sm:text-lg md:text-xl max-w-xl mb-8 sm:mb-12 leading-relaxed">
          Plan, budget, and manage your guests in one clean dashboard. Built for teams who care about the details.
        </p>
        <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 w-full sm:w-auto">
          {user ? (
            <Link to="/dashboard" className="w-full sm:w-auto bg-blue-600 hover:bg-blue-500 text-white text-base font-medium px-8 py-4 rounded-xl transition-colors shadow-sm">
              Go to Dashboard →
            </Link>
          ) : (
            <>
              <Link to="/signup" className="w-full sm:w-auto bg-blue-600 hover:bg-blue-500 text-white text-base font-medium px-8 py-4 rounded-xl transition-colors shadow-sm">
                Get Started →
              </Link>
      ...
      <section className="max-w-5xl mx-auto w-full px-4 sm:px-6 pb-16 sm:pb-24">
        <div className="grid md:grid-cols-3 gap-6">
          {features.map(({ icon, title, description }) => (
            <div key={title} className="border border-slate-200 rounded-xl p-5 sm:p-6 bg-slate-50">
